...
\caption{Representative studies on EEG feature extraction techniques for emotion recognition (with domain categories).}
...
Table~\ref{tab:eeg_feature_extraction_summary} proposes a synthesis of prevailing EEG feature extraction paradigms in the area of emotion recognition, recognizing and categorizing existing strategies with regard to the prevailing signal domains they employ. Thus, the prevailing early and light feature extraction strategies applied in the early stages of emotion recognition research relied heavily on signal representations with regard to the spectral domain, applying the Fourier transform or differential entropy. In contrast, the spatial domain-based prevailing feature extraction paradigms in the area emphasize filtering strategies based on common spatial patterns.

More recent studies are beginning to exploit these hybrid spatial-temporal-spectral representations to appropriately capture the intricate and non-linear nature of emotional brain activity. Approaches involving three-dimensional feature volume representations, attention-based techniques, and deep convolutional or transformer-based methods combine multi-modal information from different domains, contributing to even more robust and accurate emotion decoding. Spatiotemporal sequence modeling techniques, like Bi-GRU and conformer-based structures, involve encoding the dynamic emotion changes across time while preserving spatial information, which has been demonstrated to be quite powerful especially for continuous changes. Notably, all these are usually achieved with increased computational costs and the risk of overfitting.
