...
A central barrier to \emph{subject-independent} EEG emotion recognition is \textbf{domain shift}: the statistical mismatch between data used for training (source domain) and data observed at deployment (target domain). In EEG emotion tasks, a ``domain'' commonly corresponds to a \emph{subject}, \emph{session/day}, or an entirely different \emph{dataset/device montage}. Because EEG is highly nonstationary and strongly individualized, the feature distribution and even class-conditional structure can drift across subjects and recording contexts. Formally, letting the labeled source set be $\mathcal{D}_s=\{(x_i^s,y_i^s)\}_{i=1}^{n_s}$ and the target set be $\mathcal{D}_t=\{x_j^t\}_{j=1}^{n_t}$ (often unlabeled in practice), domain shift implies $P_s(X,Y)\neq P_t(X,Y)$, e.g., due to changes in $P(X)$ (covariate shift), $P(Y)$ (label shift), or $P(Y|X)$ (concept drift). Consequently, models optimized only for empirical risk on $\mathcal{D}_s$ can fail under leave-one-subject-out (LOSO) testing. \textbf{Domain adaptation (DA)} addresses this by learning representations and/or decision rules that transfer across domains, typically by aligning source and target distributions while preserving emotion-discriminative structure \cite{ganin2016dann,gretton2012kernel,sun2016deepcoral}.

\vspace{2pt}
\noindent\textbf{Taxonomy of DA in EEG emotion recognition.}
Method families used in affective EEG map closely to general DA, but with EEG-specific choices (e.g., differential entropy features, graph-structured channels, multi-source subjects). (i) \emph{Instance reweighting} adjusts the contribution of source samples to approximate the target distribution (importance weighting), partially correcting covariate shift; this is attractive when feature spaces are stable but subject distributions differ. (ii) \emph{Subspace alignment / projection-based DA} learns a shared latent space where domains become closer (e.g., kernel subspace transfer such as Transfer Component Analysis, TCA) \cite{pan2011tca}, or EEG-specific efficient alignment (e.g., adaptive subspace feature matching for cross-subject/session emotion recognition) \cite{chai2017asfm}. (iii) \emph{Discrepancy-based feature alignment} minimizes an explicit distance between source and target features, such as maximum mean discrepancy (MMD) rooted in RKHS mean embeddings \cite{gretton2012kernel} or correlation alignment (CORAL) that matches second-order statistics \cite{sun2016deepcoral}. (iv) \emph{Adversarial DA} learns \emph{domain-invariant} features by training a domain discriminator against the feature extractor (Domain-Adversarial Neural Networks, DANN) \cite{ganin2016dann}, and has been repeatedly adapted to EEG emotion settings, including two-level strategies combining discrepancy and adversarial alignment \cite{bao2021tdann}. (v) \emph{Multi-source DA} treats each source subject/session as a separate domain to reduce negative transfer; MS-MDA is a representative approach for cross-subject/cross-session EEG emotion recognition that performs one-to-one adaptation per source branch \cite{chen2021msmda}. (vi) \emph{Transfer learning / fine-tuning} leverages pretraining to initialize models, then adapts with limited target data or weak supervision; recent cross-dataset transfer work explores pretrained encoders plus alignment losses and discriminators to generalize across datasets \cite{wang2025pesd}. (vii) \emph{Few-shot and meta-learning} aims to learn initializations or update rules that rapidly adapt with a handful of target samples; model-agnostic meta-learning (MAML) is a common conceptual template for such fast adaptation \cite{finn2017maml}.

\begin{align}
\min_{G,F}\max_{D}\;\; \mathcal{L}_{\text{DANN}}
&=\mathcal{L}_y(\mathcal{D}_s;G,F)\;-\;\lambda\,\mathcal{L}_d(\mathcal{D}_s,\mathcal{D}_t;G,D), \label{eq:dann} \\
\mathrm{MMD}^2(\mathcal{Z}_s,\mathcal{Z}_t)
&=\Big\|\frac{1}{n_s}\sum_{i=1}^{n_s}\phi(z_i^s)\;-\;\frac{1}{n_t}\sum_{j=1}^{n_t}\phi(z_j^t)\Big\|_{\mathcal{H}}^2, \label{eq:mmd}\\
\mathcal{L}_{\text{CORAL}}
&=\frac{1}{4d^2}\,\|C_s-C_t\|_F^2. \label{eq:coral}
\end{align}
In \eqref{eq:dann}, $G$ is a feature extractor mapping $x\mapsto z$, $F$ is the emotion classifier, and $D$ is a domain discriminator; $\mathcal{L}_y$ is the supervised emotion loss on labeled source data and $\mathcal{L}_d$ is a domain classification loss that encourages $z$ to be indistinguishable across domains via adversarial training (often implemented with a gradient reversal layer) \cite{ganin2016dann}. In \eqref{eq:mmd}, $\phi(\cdot)$ maps features into an RKHS $\mathcal{H}$, making MMD a kernel-based measure that aligns mean embeddings of source/target features \cite{gretton2012kernel}. In \eqref{eq:coral}, $C_s$ and $C_t$ are the empirical covariance matrices of source/target features (dimension $d$), and CORAL reduces shift by matching second-order statistics \cite{sun2016deepcoral}.

\noindent\textbf{Evaluation protocols and metrics.}
DA methods are typically assessed under \emph{LOSO} (train on all but one subject and test/adapt on the held-out subject) and cross-session protocols (train on one session/day, adapt to another). More challenging \emph{cross-dataset transfer} evaluates robustness to device, montage, and stimulus differences \cite{wang2025pesd}. Standard metrics include classification accuracy (often reported for valence/arousal binaries), macro-F1 for class imbalance, and balanced accuracy when per-class recall parity matters; reporting variance across folds is particularly important in LOSO due to heterogeneous subjects.

\noindent\textbf{Practical challenges and mitigation.}
EEG DA must contend with (i) nonstationarity and temporal drift, (ii) very limited labeled target data (or none), (iii) strong inter-subject variability that can cause negative transfer, and (iv) sensor/montage mismatch that complicates feature alignment. Effective mitigations include strong regularization and early stopping to prevent overfitting to noisy pseudo-labels, subject-aware normalization/alignment in feature space, and augmented training (e.g., mixup-style perturbations or noise models) to prevent the discriminator from exploiting trivial domain cues \cite{chai2017asfm,bao2021tdann,chen2021msmda}. When small labeled calibration is available, hybrid semi-supervised strategies can stabilize conditional alignment; when not, robust marginal alignment plus structured inductive biases (topology-aware encoders, multi-source weighting) can improve invariance \cite{li2020latentda,chen2021msmda}.

\begin{enumerate}
  \item \textbf{Report protocols precisely:} specify LOSO splits, whether target labels are used (unsupervised vs.\ semi-supervised), and any calibration budget.
  \item \textbf{Diagnose shift explicitly:} visualize/quantify domain discrepancy (e.g., MMD, covariance shift) before and after adaptation to validate that ``adaptation'' is occurring.
  \item \textbf{Control negative transfer:} include ablations for multi-source weighting/selection and monitor per-subject failures rather than only mean accuracy.
  \item \textbf{Stress-test generalization:} complement LOSO with at least one cross-dataset setting to capture device/montage shift \cite{wang2025pesd}.
\end{enumerate}
